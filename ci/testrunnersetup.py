...
class TestRunnerSetup(Base):
    ...
    def run(self):
        ...
        if not os.path.exists(tester_path):
            submit_pw_check(self.pw, self.patch_1, self.name, Verdict.FAIL,
                            "BlueZ Make FAIL: No test-runnder found",
                            None, self.dry_run)
            self.add_failure_end_test("No test-runnder found")
        self.log_info("Build BlueZ success")

        self.log_info("Build kernel")
        try:
        ...
        if not os.path.exists(bzimage_path):
            submit_pw_check(self.pw, self.patch_1, self.name, Verdict.FAIL,
                            ...
                            None, self.dry_run)
            self.add_failure_end_test("No bzImage found")
        self.log_info("Build kernel success")
